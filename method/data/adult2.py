...
name = "adult"
protectedIndex = 1  # race
protectedValue = 0  # black
...
def processLine(line):
    ...
    # put race second so I know what index it is
    point = ([int(age), 0 if race == 'Black' else 1] + vectorize(employer, employers) + [int(education)] +
             vectorize(marital, maritals) + vectorize(occupation, occupations) + vectorize(sex, sexes) +
